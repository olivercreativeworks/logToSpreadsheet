 ...
 * The script uses a lock to help prevent race conditions when writing to your log. If the lock times out and an instance of your script is unable to write messages to the log, those messages will be stored in this library's properties service as 'pending messages'.
 * 
 * #### Handling pending messages
  ...
  * ***You are expected to implement a function in the global scope of your script that will handle clearing out these pending messages and writing them to your log**. (See the usage section above for an example of how to do this.)
  ...
  The name of this global function should be passed as an argument to this script's ```commitToLog``` and ```commitPendingMessagesToLog``` methods. 

  This script will handle creating and removing a clock trigger that will schedule that global function to run whenever there are any pending messages.
